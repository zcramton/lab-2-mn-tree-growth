...
tree_dat %>% 
  glimpse()
```
This data table has 8 columns; treeID, standID, stand, year, species, age, inc, rad_ib. There are 131,386 rows.
...
```
There are 13,033 records of Abies balsamea and 4188 records of Pinus Strobus.
```{r}
...
```
In the last year recorded, there were 7 trees older than 200.
```{r}
# Question 5: What is the oldest tree in the dataset found using slice_max?
tree_dat %>%
  slice_max(order_by = age, n = 1)
```
The oldest tree in the data set is treeID = 24, a Pinus resinosa at 269 yoa.
```{r}
...
```
The oldest five trees in measured in 2001 include three Fraxinus nigra (FRNI) individuals and two Pinus resinosa (PIRE) individuals. Oldeds to youngest: 263 (PIRE), 259 (PIRE), 212 (FRNI), 206 (FRNI), 206 (FRNI).
```{r}
...
tree_dat %>%
  ...
  tally ()
```
There are 687 trees recorded in a 30% sample of 2002.
```{r}
...
```
The three largest trees in stand 5 when measured in 2007 were (in descending order) tree ID: 128, 157, 135.
```{r}
...
```
The three smallest trees measured in stand 3 during 2007 were (from smallest to largest) tree ID: 50, 56, 36. The range of the three smallest trees was 47.396 to 54.925 mm.
```{r}
...
tree_dat_exp %>%   
  ...
  mean()
```
The mean basal area (m^2) of Populus tremuloides (POTR) from 2007 is 0.0369 m^2.
```{r}
...
```
In 2007 there were 0 seedlings, 252 saplings, 1963 poles and 76 sawlogs recorded.
```{r}
...
```
The mean diameter at breast height was 16.094 cm and the mean standard deviation of diameter at breast height was 6.139.
```{r}
...
tree_dat_2003 <- tree_dat %>% 
  ...
  mean_species_age <- tree_dat_2003 %>% 
    group_by(species) %>% 
    summarize(mean_age = mean(age, na.rm = TRUE)) # Calculate mean age excluding null values.

# Sort by mean age in descending order and identify the three species with the oldest (highest) means.
oldest_3_species <- mean_species_age %>% 
...
oldest_3_species
```
The three oldest species are Thuja occidentalis, Fraxinus nigra and Pinus strobus with mean ages of 126.6, 83.1 and 73.3 respectively.
```{r}
...
```
There are 111 unique years with records starting in 1897 and ending in 2007.
```{r}
...
```
The five following stands have data from all 111 years: A1, D1, D2, D2, F1.
```{r}
...
# Botanical sketch of PIRE, the fastest growing species in the database.
!["Botanical sketch of Pinus resinosa, red pine, the fastest growing species in the data base."](images/pinus_resinosa.png){.center}
```
